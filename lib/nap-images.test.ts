...
describe("heading image catalog", () => {
  ...
  it("maps H1/H2 copy to the right neighborhood or service photo", () => {
    expect(
      imageForHeading("Berkshire Hathaway HomeServices Summerlin").key,
    ).toBe("summerlin");
    expect(imageForHeading("Contact Dr. Jan Duffy").key).toBe(
      "office-lake-mead",
    );
    expect(imageForHeading("55+ Communities in Las Vegas").key).toBe(
      "active-adult-clubhouse",
    );
    expect(imageForHeading("First-Time Home Buyers").key).toBe(
      "first-time-buyer",
    );
    expect(imageForHeading("Golf Course Communities").key).toBe("golf-course");
    expect(imageForHeading("Del Webb at Lake Las Vegas").key).toBe(
      "lake-las-vegas",
    );
    expect(imageForHeading("Luxury Pool Estates").key).toBe(
      "luxury-pool-terrace",
    );
    expect(imageForHeading("MacDonald Highlands").key).toBe(
      "macdonald-highlands",
    );
    ...
    expect(imageForHeading("Divorce & Probate").key).toBe(
      "divorce-probate-desk",
    );
    expect(imageForHeading("Lock-and-Leave Condos").key).toBe(
      "lock-and-leave-condo",
    );
    expect(
      imageForHeading("Your First Home in Las Vegas Starts Here").key,
    ).toBe("first-time-buyer");
    expect(
      imageForHeading("Why Choose Berkshire Hathaway HomeServices?").key,
    ).toBe("why-bhhs-office");
    expect(imageForHeading("Ready for Your Next Chapter?").key).toBe(
      "move-up-home",
    );
    expect(imageForHeading("Difficult Situations, Expert Guidance").key).toBe(
      "divorce-probate-desk",
    );
    expect(imageForHeading("Frequently Asked Questions").key).toBe(
      "faq-consultation",
    );
    expect(imageForHeading("What's Your Las Vegas Home Worth?").key).toBe(
      "home-valuation-cma",
    );
    expect(imageForHeading("Las Vegas Real Estate Services").key).toBe(
      "real-estate-services",
    );
    expect(imageForHeading("Las Vegas Luxury Real Estate").key).toBe(
      "luxury-pool-terrace",
    );
    expect(imageForHeading("New Opportunity? We Handle Your Sale.").key).toBe(
      "seller-sold-home",
    );
    expect(imageForHeading("Sun City Anthem").key).toBe("anthem-henderson");
    expect(imageForHeading("Guard-Gated Communities").key).toBe(
      "guard-gated-entry",
    );
    expect(imageForHeading("Dr. Jan Duffy").key).toBe("office-lake-mead");
    expect(imageForHeading("Ask in person at Lake Mead Boulevard").key).toBe(
      "lake-mead-blvd",
    );
    expect(imageForHeading("Las Vegas Homes for Sale").key).toBe(
      "featured-homes",
    );
    expect(imageForHeading("What Our Clients Say").key).toBe("client-reviews");
    expect(imageForHeading("Neighborhood matching in Las Vegas").key).toBe(
      "neighborhood-matching",
    );
    expect(imageForHeading("Featured Las Vegas Neighborhoods").key).toBe(
      "neighborhood-matching",
    );
...
describe("Cloudflare hosted Images delivery", () => {
  it("uses the account hash and custom ID in the imagedelivery.net URL", () => {
    ...
    expect(CLOUDFLARE_IMAGES.accountId).toBe(
